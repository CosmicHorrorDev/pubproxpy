...
RATE_LIMIT_RESP = (
    "We have to temporarily stop you. You're requesting proxies a little too fast (2+"
    " requests per second). Get your API to remove this limit at"
    " http://pubproxy.com/#premium"
)
DAILY_LIMIT_RESP = (
...
_API_KEY_ERROR_MESSAGE = "Invalid API key, make sure you're using a valid API key"
_RATE_LIMIT_ERROR_MESSAGE = (
    "You have exceeded the rate limit, this could be due to multiple programs/threads"
    " with a `ProxyFetcher` all attempting to use the API within the time limit. If"
    " this is not the case then sorry but the API hates you, consider raising an issue"
    f" at {_REPO_URL}"
)
